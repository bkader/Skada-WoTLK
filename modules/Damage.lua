...
do
  Skada:AddLoadableModule(damagedone, nil, function(Skada, L)
    if Skada.db.profile.modulesBlocked[damagedone] then return end
...
-- ============================= --
do
  local modname="Avoidance & Mitigation"
  Skada:AddLoadableModule(modname, nil, function(Skada, L)
    -- requires the damage taken module to be enabled.
    if Skada.db.profile.modulesBlocked[damagetaken] then return end
...
do
  ...
  -- ================== --
  do
    local modname="Enemy damage done"
    Skada:AddLoadableModule(modname, nil, function(Skada, L)
      if Skada.db.profile.modulesBlocked[modname] then return end
  ...
  -- ================== --
  do
    local modname="Enemy damage taken"
    Skada:AddLoadableModule(modname, nil, function(Skada, L)
      if Skada.db.profile.modulesBlocked[modname] then return end
