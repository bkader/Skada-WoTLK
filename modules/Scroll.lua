...
function mod.Create(self,win)
  ...
  if win.bargroup and not hooked[win.bargroup] then
    win.bargroup:HookScript("OnMouseDown", function(frame, button) if button == db.button then mod.BeginScroll(win) end end)
    hooksecurefunc(win.bargroup,"SortBars", function() mod.HookMore(win) end)
    windows[win] = true
  ...
  Skada.options.args.scrolloptions = {
    ...
    args = {
      ...
      upkey = {
        ...
        get = function(info) return GetBindingKey("SKADA_SCROLLUP") end
      },
      downkey = {
        type="keybinding",
        order=120,
        ...
        get = function(info) return GetBindingKey("SKADA_SCROLLDOWN") end
      },
    }
  }
end
