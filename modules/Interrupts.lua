...
Skada:AddLoadableModule(modname, nil, function(Skada, L)
  ...
  local function log_interrupt(set, data)
    ...
    player.interrupts.extraspells[data.extraspellname].count=player.interrupts.extraspells[data.extraspellname].count+1
  ...
  function spellsmod:Enter(win, id, label)
    ...
    if player then
      self.title=player.name..L["'s "]..L["Interrupted spells"]
    end
  end
  ...
  function targetsmod:Enter(win, id, label)
    ...
    local player=Skada:find_player(win:get_selected_set(), id)
    if player then
      self.title=player.name..L["'s "]..L["Interrupted targets"]
    end
  end
  ...
  function playermod:Enter(win, id, label)
    ...
    local player=Skada:find_player(win:get_selected_set(), id)
    if player then
      self.title=player.name..L["'s "]..L["Interrupt spells"]
    end
  end
