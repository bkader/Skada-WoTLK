...
do
    ...
    function Skada:Report(channel, chantype, report_mode_name, report_set_name, max, window)
        ...
        local title = report_mode.title or report_mode:GetName()
...
function Skada:UpdateDisplay(force)
    ...
    for _, win in ipairs(windows) do
        if (changed or win.changed) or self.current then
            ...
            -- if win.db.child and not self.child then
            -- end
    ...
    function Skada:CombatLogEvent(_, timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
        ...
        if
         ...
         then
            ...
            if src_is_interesting or dst_is_interesting then
                ...
                tentativehandle =
                    self.NewTimer(
                    self.db.profile.tentativetimer or 1,
                    function()
                        tentative = nil
                        tentativehandle = nil
                        self.current = nil
        ...
        if
            eventtype == "SPELL_SUMMON" and
                (band(srcFlags, RAID_FLAGS) ~= 0 or band(srcFlags, PET_FLAGS) ~= 0 or
                    ((band(dstFlags, PET_FLAGS) ~= 0 or band(srcFlags, SHAM_FLAGS) ~= 0) and pets[dstGUID]))
         then
