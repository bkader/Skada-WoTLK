local lib = LibStub:NewLibrary("LibUIDropDownMenu", 1.2)
...
local tonumber, max, type, wipe = tonumber, math.max, type, table.wipe
...
function L_UIDropDownMenu_CreateFrames(level, index)
	while (level > L_UIDROPDOWNMENU_MAXLEVELS) do
		...
		local newList =
			CreateFrame("Button", "L_DropDownList" .. L_UIDROPDOWNMENU_MAXLEVELS, nil, "L_UIDropDownListTemplate")
		...
		for i = L_UIDROPDOWNMENU_MINBUTTONS + 1, L_UIDROPDOWNMENU_MAXBUTTONS do
			local newButton =
				CreateFrame(
				"Button",
				"L_DropDownList" .. L_UIDROPDOWNMENU_MAXLEVELS .. "Button" .. i,
				newList,
	...
	while (index > L_UIDROPDOWNMENU_MAXBUTTONS) do
		...
		for i = 1, L_UIDROPDOWNMENU_MAXLEVELS do
			local newButton =
				CreateFrame(
				"Button",
				"L_DropDownList" .. i .. "Button" .. L_UIDROPDOWNMENU_MAXBUTTONS,
				_G["L_DropDownList" .. i],
				"L_UIDropDownMenuButtonTemplate"
			)
...
function L_UIDropDownMenu_AddButton(info, level)
	...
	if colorSwatch and info.hasColorSwatch then
...
function L_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	...
	if (listFrame:IsShown() and (L_UIDROPDOWNMENU_OPEN_MENU == tempFrame)) then
		...
		if (level == 1) then
			...
			if
				(strsub(button:GetParent():GetName(), 0, 14) == "L_DropDownList" and
					strlen(button:GetParent():GetName()) == 15)
			 then
				anchorFrame = button
			else
