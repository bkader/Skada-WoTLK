...
Skada:AddLoadableModule(damagedone, nil, function(Skada, L)
    ...
    if player then
      ...
      for mobname, amount in pairs(player.damagedone.targets) do
        ...
        d.valuetext=Skada:FormatValueText(
          ...
          format("%02.1f%%", amount/player.damagedone.amount*100), self.metadata.columns.Percent
          )
  ...
  function mod:OnEnable()
    ...
    mod.metadata={showspots=true, post_tooltip=damage_tooltip, click1=playermod, click2=targetmod, columns={Damage=true, DPS=true, Percent=true}}

    dpsmod.metadata={showspots=true, tooltip=dps_tooltip, click1=playermod, click2=targetmod, columns={DPS=true, Percent=true}}
...
Skada:AddLoadableModule(damagetaken, nil, function(Skada, L)
  ...
  function sourcemod:Update(win, set)
    ...
    if player then
      ...
      for mobname, amount in pairs(player.damagetaken.sources) do
        ...
        d.valuetext=Skada:FormatValueText(
          Skada:FormatNumber(amount), self.metadata.columns.Damage,
          format("%02.1f%%", amount/player.damagetaken.amount*100), self.metadata.columns.Percent
          )
  ...
  function mod:Update(win, set)
    ...
    for i, player in ipairs(set.players) do
      if player.damagetaken.amount>0 then
        ...
        local dtps=player.damagetaken.amount/math.max(1, totaltime)
  ...
  function mod:OnEnable()
    ...
    playermod.metadata={tooltip=spellmod_tooltip, click1=spellmod, columns={Damage=true, Percent=true}}
    sourcemod.metadata={columns={Damage=true, Percent=true}}
    mod.metadata={showspots=true, click1=playermod, click2=sourcemod, columns={Damage=true, DTPS=true, Percent=true}}
...
-- ==================== --
Skada:AddLoadableModule("Friendly Fire", nil, function(Skada, L)
  ...
  local function log_damage(set, dmg)
    ...
    if amount>0 then
      ...
      if not player.friendfire.targets[dmg.dstName] then
        player.friendfire.targets[dmg.dstName]={id=dmg.dstGUID, class=select(1, UnitClass(dmg.dstName)), amount=0}
      end
  ...
  function spellmod:Update(win, set)
    ...
    if player then
      ...
      for spellid, amount in pairs(player.friendfire.spells) do
        ...
        d.valuetext=Skada:FormatValueText(
          Skada:FormatNumber(amount), self.metadata.columns.Damage,
          format("%02.1f%%", amount/set.friendfire*100), self.metadata.columns.Percent
        )
  ...
  function mod:OnEnable()
    spellmod.metadata={columns={Damage=true, Percent=true}}
    playermod.metadata={columns={Damage=true, Percent=true}}
  ...
  function mod:SetComplete(set)
    for i, player in ipairs(set.players) do
      if player.friendfire.amount==0 then
        player.friendfire.spells=0
        player.friendfire.targets=0
      end
    end
  end
end)
